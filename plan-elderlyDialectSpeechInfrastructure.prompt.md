...
将四川方言老年人语音转换为结构化业务意图，服务于养老 SaaS 和智能硬件厂商的 **B2B 语音基础设施层**。
...
**四川方言文本模板（`data/bootstrap/templates.json`）：**

```json
{
  "CALL_NURSE": ["帮我喊哈护士", "麻烦喊个医生来嘛", "护士小姐在不在哦"],
  "CALL_FAMILY": ["给我儿子打个电话", "喊我女儿回来哈", "帮我联系下老伴儿"],
  "HEALTH_ALERT": ["我不舒服得很", "肚子痛得厉害", "头晕得慌"],
  "EMERGENCY": ["救命啊快来人", "我摔倒了痛死了", "心脏不得行了快帮忙"]
}
```
...
**四川方言规范化（`core/text_normalizer.py`）：**

```python
class SichuanDialectNormalizer:
    def normalize(self, text: str) -> str:
        # 1. 方言词汇替换（喊哈→叫一下，得很→非常）
        # 2. 去除语气词（哦/嘛/嘞/哈）
        # 3. 简化口语表达
        # 4. 保留关键医疗词汇（头晕/胸闷/摔倒）
```
...
| 阶段 | 目标 |
|------|------|
| **Year 1** | 单方言（四川）深度突破；单场景成熟；建立语音数据资产；试点首单商业化 |
